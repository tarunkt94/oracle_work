...
sub get_instance_id{
	...
	my $plan_name=shift;
	my $cmd = "$emcli_dir/emcli submit_operation_plan  -plan_name=\"$plan_name\"";
...
sub run_and_suspend{
	...
	my $process_logDir = "$logDir/$instance_id";	
        system("mkdir -p $process_logDir");
	...
	while(1){
		...
		if($status eq ''){
			...
			print $BASIC "\n$output\n";
		        ...
		        open(my $LOG,'|-'," tee -a $logDir/master_log.log");
		        flock($LOG,2);
			...
			exit(0);
                        ...
                        open(my $LOG,'|-'," tee -a $logDir/master_log.log");
                        flock($LOG,2);
			print $LOG "\n[INFO] The plan $plan_name with num $num and instance id $instance_id has status of $status in step $step_to_stop. Failed to stop it . Not suspending it\n";
                        close($LOG);
			close($BASIC);
                        exit(1);

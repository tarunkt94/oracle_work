...
my ($relname,$hypusername,$hyppasswd,$sdiuser,$sdipasswd,$sdihost,$sdiscript,$infraObj,$taschost,$tascObj,$tascuser,$tascpasswd,$infrauser,$infrapasswd,$infrahost,%allhyps,$is_server_pool,$server_pool);
my(%templateHash,$sdiObj,$hvObj,$templatekey,$sys_img_loc,$sys_img_bi_loc,$hyp,$scriptDir,$bundle_scripts_loc,$email,$version,$HA,$system,$rac_version,$tasctl_loc,$service_name,$isstandby);
...
sub get_input{
        GetOptions('tag=s' => \$tag,
		   ...
		   'tascuser=s' => \$tascuser,
		   'tascpasswd=s' => \$tascpasswd,
		   'tasctl_loc=s' => \$tasctl_loc,
...
sub validate_input{
	...
	if(! ( $tag and $rackid and $primary_json_file and $seed_pool and $running_pool and $logDir and $dgname and $dgport and $rac_version and $email and $relname and  $isstandby)){
		print "\nMnadatory parameter missing\n";
		usage();
	}


	if((uc($isstandby) eq 'NO') or (uc($isstandby) eq 'FALSE')){
		if(!($infrauser and $infrapasswd and $bundle_scripts_loc and $HA )){
			die "\nParameters infrauser , infrapasswd , bundle_scripts_loc and HA are mandatory if provisionig primary pod\n";
		}
...
sub create_objects{
	...
	$taschost = $json_object->{SDI_INFO}{TAS_CENTRAL_HOST};
	...
	$tascObj = RemoteCmd->new(user => $tascuser,
                                   passwd => $tascpasswd,
                                   logObj => $logObj);
...
sub run_explicit_dr{
        ...
        my $file = "$scriptDir/explicit_dr.pl";
        my $dest = '/tmp';

        $tascObj->copyFileToHost(host => $taschost,
                                  file => $file,
                                  dest => $dest);
	my $cmd = "/tmp/explicit_dr.pl -tasctl_loc $tasctl_loc -service_name $service_name";

	print "\n$cmd\n";

        my $out = $tascObj->executeCommandsonRemote(cmd => $cmd,
	                                           host => $taschost);

        if (grep(/error|no such|failed|Fail/i, @$out)) {
                print "\nError in executing the command $cmd on TASC host $taschost\n";
                exit 1;
        }
...
sub run_cmd_on_sdi{
	...
	print "\nWill run the command $cmd on the SDI host $sdihost\n";
...
sub usage{
print << "EOF";
The follwoing paramters are mandatory, whether for Create Order or COFAPP:
...
For COFAPP, the follwing parameters are also mandatory:

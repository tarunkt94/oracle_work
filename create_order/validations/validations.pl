...
my($ovm_smc_props,$drmode,$mailid,$logdir,$logObj,$remoteDB,$remoteFA,$html_message,$sys_name,$scriptDir);
...
my %ovmHash = read_props_file($ovm_smc_props);
...
sub get_input{
	GetOptions('ovm_smc_props=s' => \$ovm_smc_props,
		   'drmode=s' => \$drmode,
		   'mailid=s' => \$mailid,
		   'logdir=s' => \$logdir,
...
sub validate_inputs{
	if(!($ovm_smc_props and $drmode and $mailid and $logdir and $sys_name)){
		pod2usage(2);
	}
	die "\nNo file present at $ovm_smc_props\n" unless (-f $ovm_smc_props);
	die "\nProvide valid drmode\n" if($drmode ne 'ACTIVE' and $drmode ne 'PASSIVE' );
...
sub start_test_suite{
	...
	$ovmHash{'sun_pswd'} = $ovmHash{'faovm.storage.sun.password'};
	$ovmHash{'sun_proj'} = $ovmHash{'faovm.storage.sun.project'};
	$ovmHash{'sun_storage'} = $ovmHash{'faovm.storage.sun.host'};
	
	$html_message = '<table width = 100% border=0><tr><td><h2><u><center>DR VALIDATIONS Report</center></u></h2></td></tr></table><br><b>DR MODE: </b>' ;

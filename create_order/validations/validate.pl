...
my($p_ovm_smc_props,$s_ovm_smc_props,$action, $drmode,
   $trusted_host, $trusted_user, $trusted_passwd,$replication_label, 
   $mailid, $logdir,$logObj, $remoteDB, $remoteFA,$html_message,
   $sys_name,$scriptDir,%ovmHash, $help);


BEGIN {
    push @INC, "$ENV{SAASQA_HOME}/pm";
    $scriptDir = "$ENV{SAASQA_HOME}/bin";
}
...
sub get_input{
    GetOptions('p_ovm_smc_props=s' => \$p_ovm_smc_props,
		   ...
		   'replication_label=s'=> \$replication_label,
	       'mailid=s'	=> \$mailid,
	       'logdir=s'	=> \$logdir,
           'help'		=> \$help,
	       'sys_name=s' => \$sys_name) or pod2usage(q(-verbose) => 1);
}
...
sub prepare_hash{
	...
	$ovmProps{'fa_password'} = $ovmHash{'faovm.os.apps.user.password'};
...
sub start_test_suite{
	...
	my $db_password = $p_ovmHash{'faovm.smc.fusiondb.new.host.login.user.password'};
	if(not defined $db_password or $db_password eq ""){
		$db_password = 'oracle';
	}
	...
	my $fa_user_name = $p_ovmHash{'faovm.os.apps.user.name'};
	my $fa_password = $p_ovmHash{'faovm.os.apps.user.password'};
	if(not defined $fa_password or $fa_password eq ""){
		$fa_password = 'Welcome1';
	}
    $remoteFA = new RemoteCmd(user => $fa_user_name,
			      passwd => $fa_password,
			      logObj => $logObj);
...
sub check_mounts{
	...
	{	run_test($key,"$scriptDir/check_mount.pl",
			 $ovmHash{$key},$remoteFA,
			 project_name =>  $ovmHash{'sun_proj'},
			 drmode => $drmode,
		     logdir => $params{logdir});
		
		generate_msg($key,"Verify mount points and fstab entries on AUX SCALE $ovmHash{$key} host",$params{logdir});
	    }		
    }	
}
...
sub check_dg_lsnr_stat{
  ...
  generate_msg('lnsr_dg_fa_rac1',"Check DG Listener status and DGMGRL/DGB services on $ovmHash{'rac_node1'}",$params{logdir});
    ...
    run_test('lnsr_dg_oid_rac1',"$scriptDir/listener_dg_check.pl",
	     $ovmHash{'rac_node1'},$remoteDB,
	     GRID_HOME => $ovmHash{'oiddb_grid_home'},
	     db_uniq_name => $ovmHash{'oiddb_uniq_name'},
	     podtype => $podtype,
		 logdir => $params{logdir});
...
sub check_proj_status{
    ...
    $cmd .= " -trusted_host $params{trusted_host} -trusted_user $params{trusted_user} ";
    $cmd .= " -trusted_passwd $params{trusted_passwd} -project $ovmHash{'sun_proj'} ";
    ...
    elsif($drmode eq 'ACTIVE' and $status =~ /PROJ_NOT_AVAILABLE/){
	system("touch $params{logdir}/project_status.dif");
    }
    elsif($drmode eq 'PASSIVE' and $status =~ /PROJ_NOT_AVAILABLE/){
	system("touch $params{logdir}/project_status.suc");
    }
    elsif($drmode eq 'PASSIVE' and $status =~ /PROJ_AVAILABLE/
...
sub check_proj_clone_status{
	...
	if($status =~ /PROJ_AVAILABLE/){	
		...
		elsif($clonestatus eq 'YES' and $status =~ /LOCAL_PROJECT/){
		system("touch $params{logdir}/project_clone_status.dif");
		}
		elsif($clonestatus eq 'NO' and $status =~ /LOCAL_PROJECT/){
		system("touch $params{logdir}/project_clone_status.suc");
		}
		elsif($clonestatus eq 'NO' and $status =~ /CLONED_PROJECT/){
		system("touch $params{logdir}/project_clone_status.dif");
		}
		else{
		system("touch $params{logdir}/project_clone_status.dif");
	...
	elsif($status =~ /PROJ_NOT_AVAILABLE/)
	{
		if($clonestatus eq 'YES'){
		system("touch $params{logdir}/project_clone_status.dif");
		}
		elsif($clonestatus eq 'NO'){
		system("touch $params{logdir}/project_clone_status.suc");
		}
	}
...
sub check_proj_nfs_exceptions_status{
    ...
    my $cmd = "perl $scriptDir/project_status.pl -wdir $params{logdir} ";
    $cmd .= " -trusted_host $params{trusted_host} -trusted_user $params{trusted_user} ";
    $cmd .= " -trusted_passwd $params{trusted_passwd} -project $p_ovmHash{'sun_proj'} ";
...
sub check_proj_replication_status{
    ...
    $cmd .= " -trusted_passwd $params{trusted_passwd} -project $ovmHash{'sun_proj'} ";
    $cmd .= " -filer $ovmHash{'sun_storage'} -filer_user $ovmHash{'sun_username'} ";
    $cmd .= " -replication_label $params{replication_label} | tee $params{logdir}/check_proj_replication_status.log";
    
    my $status = `$cmd`;
    print "\n$status\n";
    if($drmode eq 'ACTIVE' and $status =~ /true/){
	system("touch $params{logdir}/project_repl_status.suc");
    }
    elsif($drmode eq 'PASSIVE' and $status =~ /false/){
	system("touch $params{logdir}/project_repl_status.suc");
    }
    else{
...
=head1 NAME
    ...
    Validations Post Deployment Procedure Run 
...
=head1 SYNOPSIS
    ...
    Options:
        ...
        *p_ovm_smc_props:
            Primary Pod OVM property file for SMC domain. Example: fadrsdihcmser1010101

        *s_ovm_smc_props:
            Standby Pod OVM property file for SMC domain. Example: fadrsdihcmser1010101

        *trusted_host:
            Trusted Host Name for connecting to NFS Storage. 

        *trusted_user:
            User Name for Trusted Host Name used to connect into NFS Storage.

        *trusted_passwd:
            Password for Trusted Host Name used to connect into NFS Storage.

        *mailids:
            Complete email address to alert the mails. Example: fadr_qa_ops_in_grp@oracle.com

        *logdir:
            Log directory to save *.log, *.dif.... Example: /scratch/aime/mytest
        
	*sys_name
	    System Name of the DR Pair.

        *action
           This indicates the Validations to be done on respective DR DP.
	   Valid Options are:
		FA_DR_DP
		FA_DR_CLEANUP_DP
		FA_DR_CLONE_DP
		FA_DR_CLONECLEAN_DP
		FA_DR_SCALE_DP
		FA_DR_CONFIGURATION_DP
		pause
		resume
		replay replay_clone		

        -help:
            brief description about script and usage.
